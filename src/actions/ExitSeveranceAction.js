...
import { UPDATE_ITNOC, UPDATE_HRNOC, UPDATE_ADMINNOC, VIEW_SEVERANCE, INSERT_FEEDBACK, UPDATE_RELIEVING, UPDATE_FINANCENOC } from '../utils/Constants'
...
export const UpdateFinaceNoc = (checked, emp_id) => async dispatch => {
    try {

        axios({
            method: 'POST',
            url: apiurl + 'update_finance_noc',
            data: {

                "employee_id": emp_id,
                "finance_noc_date": checked === true ? moment().format('YYYY-MM-DD HH:m:s') : "",
                "finance_noc_by": checked === true ? localStorage.getItem("empId") : ""
            }
        })
            .then((response) => {
                if (response.data.status === 1) {
                    notification.success({
                        message: "FINANCE NOC approved successfully",
                    });
                    dispatch({ type: UPDATE_FINANCENOC, payload: response.data.status })
                    dispatch(GetSeverance(emp_id))
                    dispatch(getOtherTask())
                }
                return Promise.resolve();

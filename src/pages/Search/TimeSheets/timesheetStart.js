...
function TimeSheetStartModel(props) {
  ...
  const submitStartTimeSheet = async () => {
    ...
    if (filtererr.length > 0) {
      ...
      timeSheetForm.activity.value === 6 &&
      AddHearing_Data.length === 0
    ) {
      ...
      if (result1) {
        let startTime = dateFormat(
          timeSheetForm.startTime.value && timeSheetForm.startTime.value != ""
            ? timeSheetForm.startTime.value
            : new Date()
        );
        let end_time = dateFormat(
          timeSheetForm.endTime.value && timeSheetForm.endTime.value != ""
            ? timeSheetForm.endTime.value
            : new Date()
        );
        ...
        var insert_data = {
          project_id:
            (props.approve_timesheet &&
              props.approve_timesheet !== "" &&
              props.approve_timesheet.project_id) ||
            rowId ||
          ...
          end_date: EndDateTimeShow
            ? moment(timeSheetForm.fromDate.value, "YYYY-MM-DD").format(
                "YYYY-MM-DD"
              )
            : "0000-00-00",
  ...
  const onEditTimesheet = async () => {
    ...
    var timesheetData = {
      timesheet_id:
        props.project_wise_edit && props.project_wise_edit[0].timesheet_id,
      project_id:
        timeSheetForm.projectname.value === ""
          ? 0
          : timeSheetForm.projectname.value,
    ...
    await dispatch(
      EditProjectwiseTimesheet(timesheetData, props.project_wise_edit[1])
    );
  ...
  useEffect(() => {
    if (
      props.timeSheetProject.length > 0 ||
      props.getTaskTimeSheet.length > 0 ||
      props.project_wise_reject ||
      props.project_wise_edit ||
      props.project_wise
    ) {
      ...
      if (data) {
        ...
        timeSheetForm.startTime.value = new Date(
          "12-30-2017 " + data.start_time
        );
        ...
        data.task_status && data.task_status === "Completed"
          ? (timeSheetForm.task_status.value = 1)
          : (timeSheetForm.task_status.value = 0);
        if (
          props.project_wise_edit &&
          data.end_date !== "0000-00-00" &&
          data.end_time !== "00:00:00"
          ...
          data.end_time &&
            (timeSheetForm.endTime.value = new Date(
              "12-30-2017 " + data.end_time
            ));
        ...
        if (
          (data.end_date === "0000-00-00" && data.end_time === "00:00:00") ||
          (!data.end_date && !data.end_time && !props.project_wise_edit)
        ) {
  ...
  useEffect(() => {
    if (!props.project_wise_edit) {
      ...
      if (props.getLastTimeSheet.length > 0) {
        let startTime =
          props.getLastTimeSheet[0]?.end_time !== "00:00:00"
            ? new Date("12-30-2017 " + props.getLastTimeSheet[0]?.end_time)
            : moment(props.getLastTimeSheet[0]?.start_time, "HH:mm").add(
                5,
                "minute"
              );
        ...
        setTimeSheetForm((prevState) => ({
          ...
          endTime: {
            ...timeSheetForm.endTime,
            value: moment(startTime, "HH:mm").add(5, "minute"),
          },
        }));
        ...
        setTimeSheetForm((prevState) => ({
          ...
          endTime: {
            ...timeSheetForm.endTime,
            value: moment(new Date(), "HH:mm").add(5, "minute"),
          },
        }));
      }
    }
        ...
        <div>
          <Grid item xs={12} container direction="row" spacing={3}>
            ...
            {props.approve_timesheet && props.approve_timesheet !== "" && (
              <>
                <Grid item xs={4}>
                  {props.approve_timesheet.project_type}
                </Grid>
                <Grid item xs={4}>
                  {props.approve_timesheet.project_name}
                </Grid>
                <Grid item xs={4}>
                  {props.approve_timesheet.client}{" "}
                </Grid>
              </>
            )}

            {(props.project_wise || props.project_wise_edit) && (
              <>
                {" "}
                <Grid item xs={4} container direction="column">
                  ...
                  <Labelbox
                    ...
                    disabled={timeSheetForm.projectname.disabled}
                  ></Labelbox>
                </Grid>
                ...
                <Grid item xs={3}>
                  <Labelbox
                    ...
                    disabledHours={Array.from(
                      Array(
                        Number(
                          moment(timeSheetForm.startTime.value, "HH:mm")
                            .add(5, "minute")
                            .format("HH")
                        )
                      ),
              ...
              {timeSheetForm.activity.value === 6 && (
                <>
                  <img
                    src={Order}
                    alt="hearing_project"
                    className="hearing_project"
                    onClick={() => fntaskHearingDetails()}
                  />
                  &nbsp;
                  <label>
                    {props.project_wise_edit ? "Edit" : "Add"} Hearing
                  </label>
                </>
              )}
            </div>

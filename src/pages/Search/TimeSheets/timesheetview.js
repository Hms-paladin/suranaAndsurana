...
function TimeSheetView(props) {
  ...
  const tableRows = [
    {
      start_time: "09:30 AM",
      end_time: "10:30 AM",
      activity: "Pre-Bono Work",
      sub_activity: "Prebono",
      comments: "",
      action: (
    ...
    },
    {
        start_time: "10:30 AM",
        end_time: "11:30 AM",
        activity: "Article Publications",
        sub_activity: "Sub Activity Test",
        comments: "",
        action: (
          <div className="DataName">
            <img
  ...
  async function submitStartTimeSheet() {
    var result1 = true;
    if (!FinalCheckTimeOverlap) {
      result1 = await Todateopen(
        timeSheetForm.fromDate.value,
        timeSheetForm.startTime.value,
        timeSheetForm.endTime.value
      );
      setFinalCheckTimeOverlap(true);
    }
    if (result1) {
      ...
      var timesheetData = {
        ...
        start_date: moment(timeSheetForm.fromDate.value, "YYYY-MM-DD").format(
          "YYYY-MM-DD"
        ),
        start_time:
          timeSheetForm.startTime.value !== null &&
          timeSheetForm.startTime.value !== ""
            ? moment(timeSheetForm.startTime.value).format("HH:mm")
            : moment(new Date()).format("HH:mm"), //dateFormat(timeSheetForm.startTime.value != undefined ? timeSheetForm.startTime.value : new Date(), "hh:MM:ss"),
        comment: timeSheetForm.description.value,
        created_by: localStorage.getItem("empId"),
      };
      if (EndDateTimeShow) {
        timesheetStopData = {
          end_date: moment(timeSheetForm.fromDate.value, "YYYY-MM-DD").format(
            "YYYY-MM-DD"
          ),
          end_time:
            timeSheetForm.endTime.value !== null &&
            timeSheetForm.endTime.value !== ""
              ? moment(timeSheetForm.endTime.value).format("HH:mm")
              : moment(new Date()).format("HH:mm"), //dateFormat(timeSheetForm.endTime.value != undefined ? timeSheetForm.endTime.value : new Date(), "hh:MM:ss"),
          comment: timeSheetForm.description.value,
  ...
  async function submitStopTimesheet() {
    ...
    if (result) {
      ...
      dispatch(insertTimeSheetbyTime(timesheetData, false, props.rowData)).then(
        (response) => {
          dispatch(getTaskTimeSheet(props.rowData.task_id)).then((response) => {
  ...
  const TimeSheetValidation = async (from_date, start_time, end_time) => {
    ...
    if (
      (TimeSheetStartProcess === true && EndDateTimeShow === true) ||
  ...
  const Todateopen = async (from_date, start_time) => {
    ...
    timesheet_statement = await TimeSheetValidation(
      from_date,
      start_time,
      timeSheetForm.endTime.value
    );
  ...
  useEffect(() => {
    ...
    if (
      ...
      props.getTaskTimeSheet[0] &&
      props.getTaskTimeSheet[0].task_id == props.rowData.task_id
    ) {
      if (props.getTaskTimeSheet.length > 0) {
        if (props.getTaskTimeSheet[0].timesheet.length > 0) {
          ...
          if (
            props.getTaskTimeSheet[0].timesheet[tsSize].start_date == null &&
            props.getTaskTimeSheet[0].timesheet[tsSize].start_time == null
          ) {
            setTimeSheetStartProcess(true);
            settimeSheetID(
              props.getTaskTimeSheet[0].timesheet[tsSize].timesheet_id
            );
          } else if (
            props.getTaskTimeSheet[0].timesheet[tsSize].end_date != null &&
            props.getTaskTimeSheet[0].timesheet[tsSize].end_date !==
              "0000-00-00" &&
            props.getTaskTimeSheet[0].timesheet[tsSize].end_time != null &&
            props.getTaskTimeSheet[0].timesheet[tsSize].end_time !== "00:00:00"
          ) {
            var timeObj = {
              activity_id: null,
              end_date: null,
            ...
            setTimeSheetStartProcess(true);
            settimeSheetID(
              props.getTaskTimeSheet[0].timesheet[tsSize + 1].timesheet_id
            );
          } else {
            setTimeSheetStartProcess(false);
            ...
            setstartDateDisplay(
              moment(
                props.getTaskTimeSheet[0].timesheet[tsSize].start_date
              ).format("DD MMM YYYY")
            );
            timeSheetForm.fromDate.value =
              props.getTaskTimeSheet[0].timesheet[tsSize].start_date;
            timeSheetForm.startTime.value = moment(
              props.getTaskTimeSheet[0].timesheet[tsSize].start_time,
              ["HH.mm"]
            );
            timeSheetForm.description.value =
              props.getTaskTimeSheet[0].timesheet[tsSize].description;
            var startttime = moment(
              props.getTaskTimeSheet[0].timesheet[tsSize].start_time,
              ["HH.mm"]
            ).format("HH:mm:ss");
        ...
        <>
          <Grid item xs={12} container direction="row">
            <Grid item xs={4} container direction="column" spacing={1}>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
                  {props.rowData.project_type != null
                    ? props.rowData.project_type
                    : "Adhoc Task"}
                </div>
              </Grid>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
                  {props.rowData.activity != null
                    ? props.rowData.activity
                    : props.rowData.tag}
                </div>
              </Grid>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>{props.rowData.Priority}</div>
              </Grid>
            </Grid>
            <Grid item xs={4} container direction="column" spacing={1}>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
                  {props.rowData.project_name != null
                    ? props.rowData.project_name
                    : props.rowData.description}
                </div>
              </Grid>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>{props.rowData.sub_activity}</div>
              </Grid>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
                  {props.rowData.project_type != null ? props.rowData.tag : ""}
                </div>
              </Grid>
            </Grid>
            <Grid item xs={4} container direction="column" spacing={1}>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
                  {props.rowData.client != null
                    ? props.rowData.client
                    : props.rowData.assignee_name}
                </div>
              </Grid>
              <Grid item xs={4}>
                <div style={{ width: 250 }}>
          ...
          <Grid item xs={12} container direction="row" spacing={3}>
            ...
            <Grid item xs={3}>
              <Labelbox
                ...
                changeData={(data) => checkValidation(data, "startTime")}
              ...
              <>
                ...
                <Grid item xs={3}>
                  <Labelbox
                    ...
                    changeData={(data) => checkValidation(data, "endTime")}
          ...
          <div className="customiseButton">
            <CustomButton
              ...
              onBtnClick={closeModel}
            />
            <CustomButton
              btnName={`${!EndDateTimeShow ? "START" : "SAVE"}`}
              btnCustomColor="customPrimary"
              btnDisable={TimeOverlap || EndTimeExceed}
              custombtnCSS="timeSheetButtons"
        ...
        <>
          ...
          <div className="customiseButton">
            ...
            <CustomButton
              ...
              custombtnCSS="timeSheetButtons"
              btnDisable={TimeOverlap || EndTimeExceed}
              onBtnClick={submitStopTimesheet}
            />
          </div>
         
        </>
      )}
      <div className="timsheet_overlap">
        {TimeOverlap && "Time Overlapping"}
      </div>
      <div className="timsheet_overlap">
        {EndTimeExceed && "End Time Should not less than from time"}
      </div>
    </div>
  );
}
